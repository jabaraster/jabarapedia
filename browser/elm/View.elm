...
type alias TrueClass = String
type alias FalseClass = String

conditionClass : Bool -> Prefix -> TrueClass -> FalseClass -> Attribute msg
conditionClass cond pre t f =
  class <| if cond then (pre ++ " " ++ t) else (pre ++ " " ++ f)

conditionClass_ : Bool -> TrueClass -> FalseClass -> Attribute msg
conditionClass_ cond t f =
  class <| if cond then t else f

toggleArrow : Bool -> Html msg
toggleArrow open = 
    span [class <| "toggle-arrow" ++ if open then " toggle-open" else " toggle-close"] [text "▶︎"]

type alias Min = Int
type alias Max = Int
type alias NewValue = Int
    ...
    div [class "stars"] <|
      map (
        \idx ->
          let h = handler idx
          in  i [
                class <| "fas fa-star " ++ if idx <= cur then "star-on" else "star-off"
              , onClick h
              ] []
      ) <| range min max